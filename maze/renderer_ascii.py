...
def _bfs_visit_order(grid, width, height, entry, exit_):
    ...
    while queue:
        ...
        for direction in ALL_DIRECTIONS:
            if grid[y][x].has_wall(direction): continue
...
def _path_cells_from_letters(entry: tuple[int, int], letters: str) -> list[tuple[int, int]]:
    x, y = entry
    path = [(x, y)]
    for m in letters:
        if m == "N": y -= 1
...
def render_ascii(
    ...
    for y in range(height):
        ...
        for x in range(width):
            ...
            # Parede Oeste
            line_mid += f"{theme.wall_color}┃{theme.reset}" if cell.has_wall(Direction.WEST) else f"{bg} {theme.reset}"
        ...
        line_mid += f"{theme.wall_color}┃{theme.reset}" if grid[y][width-1].has_wall(Direction.EAST) else " "
        out_lines.append(line_mid)

        if y < height - 1:
            line_div = f"{theme.wall_color}┣"
            for x in range(width):
                line_div += "━━━" if grid[y][x].has_wall(Direction.SOUTH) else f"{theme.reset}{theme.bg_a if (x+y)%2==0 else theme.bg_b}   {theme.reset}{theme.wall_color}"
                line_div += "╋" if x < width - 1 else "┫"
            out_lines.append(line_div + theme.reset)

    out_lines.append(f"{theme.wall_color}┗" + "━━━┻" * (width - 1) + "━━━┛" + theme.reset)
    return "\n".join(out_lines)

def interactive_ascii_session(generator_factory, width, height, entry, exit_, perfect):
    show_path = True
    theme_idx = 0
    ...
    def build_and_animate():
        ...
        if show_path:
            ...
            for i in range(0, len(search_order), 3): 
                print("\033[H", end="")
                print(render_ascii(maze.grid, width, height, entry, exit_, set(search_order[:i]), set(), p42, theme_idx))
                time.sleep(0.01)

            # Animação do Caminho
            for i in range(len(path_points) + 1):
                print("\033[H", end="")
                print(render_ascii(maze.grid, width, height, entry, exit_, set(search_order), set(path_points[:i]), p42, theme_idx))
                time.sleep(0.04)
    ...
    while True:
        print("\033[H", end="")
        print(render_ascii(grid, width, height, entry, exit_, 
                          set(search_order) if show_path else set(), 
                          set(path_points) if show_path else set(), 
                          p42, theme_idx))
        print(f"\n{THEMES[theme_idx%2].wall_color}R: New | P: Path | C: Color | Q: Quit{THEMES[theme_idx%2].reset}")
        cmd = input("> ").strip().lower()
        if cmd == "q": break
        elif cmd == "c": theme_idx = (theme_idx + 1) % len(THEMES)
        elif cmd == "p": show_path = not show_path
        elif cmd == "r": grid, search_order, path_points, p42 = build_and_animate()